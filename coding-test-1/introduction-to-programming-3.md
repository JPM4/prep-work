...
# prints 2
puts(item)
# prints 3, which is now the item at position zero
puts(arr[0])
...
There's a lot more to Ruby than what we've covered here. Probably the
biggest thing we've skipped is how to define new Ruby **classes**;
we've only used integers, strings and arrays, but you can write your
own new types of objects. We've also not talked about the `Hash`
class, or "blocks" in Ruby.
...
**Now do the practice problems we've provided before the coding
challenge**. You want to practice so that you do your best.
